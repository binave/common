 ...
 * @version 1.0
 */
public class DataAlter {
    private final static int BUFFER_SIZE = Integer.SIZE;// 缓冲池最大位数
    private final static int CUT_SIZE = Byte.SIZE;// 默认切除位数
    /* 用于保证二进制位移统一性 */
     ...
     *         如果返回值不大于 0 ，则无法继续存入
     */
    int offer(int num, int offset) throws RuntimeException {
        try {
            if (offerIndex == data.length)// 已到数组末尾，无法再加入数值
                return 0;
        } catch (Exception e) {
            return 0;// 空数组，无长度
        }
    ...
    private int inBytes() {
        try {
            for (; offerBit >= CUT_SIZE; offerBit -= CUT_SIZE) {
                data[offerIndex++] = (byte) offerBuffer;// 将数值低 8 位存入数组
                offerBuffer >>>= CUT_SIZE;// 去掉低八位
            }
            return (data.length - offerIndex) * CUT_SIZE - offerBit;// 返回剩余位数
    ...
    int close() {
        try {
			 ...
			 */
            if (offerIndex < data.length && offerBit > 0)
                data[offerIndex++] = (byte) offerBuffer;
     ...
     */
    int poll(int offset) throws RuntimeException {
        ...
        try {
            while (pollBit < offset) {
				 ...
				 */
                pollBuffer += (data[pollIndex++] + PATCH_VALUE) % PATCH_VALUE << pollBit;
